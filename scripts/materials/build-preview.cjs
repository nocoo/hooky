...
async function embed(slug, view, relativeFile) {
  ...
  const bridge = slug === 'hooky' && view === 'popup' ? `import {sendWebhook,sendAnyway} from ${enc(await module(path.join(folder, 'src/send.js')))};import {getDuplicateCapture} from ${enc(await module(path.join(folder, 'src/duplicates.js')))};window.hookyPreviewSend=message=>{if(message.type==='EXECUTE_WEBHOOK')return sendWebhook(message.config,message.context,{tab:message.tab,resolved:message.resolved,source:'popup'});if(message.type==='GET_DUPLICATE_CAPTURE')return getDuplicateCapture(message.token);if(message.type==='SEND_ANYWAY')return sendAnyway(message.token);return {ok:false,error:'requestFailed'}};` : '';
  ...
  const start = `<script type="module">${bridge}${entries.map(name => `import ${JSON.stringify(name)};`).join('')}new ResizeObserver(()=>parent.resizePreview(${enc(slug)},${enc(view)},document.body.scrollHeight)).observe(document.body);</script>`;
...
const seed={hooky:{hooky:${enc(demo.store)}},r2shot:{r2config:{endpoint:'https://demo.r2.cloudflarestorage.com',bucketName:'screenshots',accessKeyId:'EXAMPLE_KEY_FOR_DEMO',secretAccessKey:'EXAMPLE_SECRET_FOR_DEMO',customDomain:'images.example.com',jpgQuality:90,maxScreens:5},r2shot_theme:'light'}};
let stores=structuredClone(seed),sessions={hooky:{},r2shot:{}},windows={hooky:{},r2shot:{}},heights={};
let context=${enc(demo.context)};
...
async function previewFetch(slug,url,options={}){log(slug,{mode:'Simulated request',url,method:options.method||'GET',headers:Object.fromEntries(Object.entries(options.headers||{}).map(([name,value])=>[name,name.toLowerCase()==='content-type'?value:'••••'])),body:options.body,status:response(slug)?403:201});return new Response(JSON.stringify(${enc(demo.receipt)}),{status:response(slug)?403:201,headers:{'Content-Type':'application/json'}})}
function storageArea(slug,view,area){return {get:async(keys)=>{const state=structuredClone(area==='local'?stores[slug]:sessions[slug]);if(!keys)return state;if(typeof keys==='string')return {[keys]:state[keys]};return Object.fromEntries(keys.map(key=>[key,state[key]]))},set:async(values)=>{const state=area==='local'?stores[slug]:sessions[slug],changes=Object.fromEntries(Object.entries(values).map(([key,value])=>[key,{oldValue:state[key],newValue:structuredClone(value)}]));Object.assign(state,structuredClone(values));for(const frame of Object.values(windows[slug]))for(const listener of frame.listeners)listener(changes,area);if(area==='local'&&view==='workspace')setTimeout(()=>refresh(slug),0)}}}
function previewAPI(slug,view,win){
 ...
 storage:{local:storageArea(slug,view,'local'),session:storageArea(slug,view,'session'),onChanged:{addListener:listener=>listeners.push(listener)}},permissions:{contains:async()=>false,request:async()=>false},action:{setBadgeText:async()=>{},setBadgeBackgroundColor:async()=>{},setTitle:async()=>{}},
 ...
 if(slug==='hooky')return win.hookyPreviewSend(message);
...
byId('reset').onclick=()=>{stores=structuredClone(seed);sessions={hooky:{},r2shot:{}};context=structuredClone(initialContext);byId('context-form').reset();for(const slug of ${enc([product])}){byId(slug+'-response').value='success';refresh(slug,'workspace');refresh(slug);log(slug,'Demo reset. Ready for another try.')}};
</script></body></html>`;
