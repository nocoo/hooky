...
function screenshot(slug, index) {
  ...
  if (index === 1 || (slug === 'hooky' && index === 2)) {
    const heading = slug === 'hooky' ? (index === 1 ? 'Your webhook. Your options.' : 'The right page. The right webhook.') : 'Your storage. Your settings.';
    const sub = slug === 'hooky' ? (index === 1 ? 'Add headers, a send UUID, and an optional receipt to each webhook.' : 'Match a URL or title. Test it. Set its priority. Send on your click.') : 'Connect your R2 bucket. Choose the quality. Set your full-page limit.';
    return {
