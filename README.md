...
- **可选权限**：桌面通知默认关闭，可在设置中选择仅错误及未确认结果或全部结果。启用时申请通知权限，拒绝授权仍可正常发送。
...
右键菜单和扩展图标的右键菜单提供“预览发送”和“最近发送结果”，这些操作不会触发快捷发送规则。同模板、同内容的请求进行中再次触发会复用正在进行的发送；请求结束后可以再次发送。请求在 20 秒后超时，超时或连接中断表示结果未确认，请先检查接收端；Hooky 不自动重试。HTTP 成功表示接收端响应成功，不保证业务数据已经持久保存。
...
[概念稿与已通过的家族设计](docs/design/README.md) · [Hooky 2.1.0 上传材料](materials/2.1.0/index.html) · [人工测试说明](materials/2.1.0/TESTING.md)
